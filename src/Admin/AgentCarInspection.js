...
export const AgentCarInspection = () => {
  ...
  useEffect(() => {
    // Retrieve agent ID from session storage
    if (!storedAgentId) {
    ...
    const fetchPosts = async () => {
      try {
        const response = await api.get(
          `/vehiclelisting/${storedAgentId}/Agent/CarForInspection`
        );
  ...
  const fetchSerchdata = async () => {
    try {
      const response = await api.post(
        `/vehiclesearch/${storedAgentId}/Agent/CarForInspection`,
        {
          search: searchQuery,
        }
      );
    ...
    <section className="car-details">
      ...
      <div className="container">
        <div className="view-post-panel mid-table mt-4">
          <h3 className="main-heading py-3">New Cars For Inspection</h3>
                ...
                {loading ? (
                  ...
                  <div className="col-md-12">
                    <p className="text-center text-danger">
                      No New Cars For Inspection
                    </p>
                  </div>
                ) : (
                  posts.map((post) => (
                    <div key={post.id} className="col-lg-4 col-12">
                      <a href={`/agent-car-details-popup/${post.Vehicle_Id}`}>
